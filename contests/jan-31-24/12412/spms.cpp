...
unordered_map<int, pair<int, pair<string, vector<double>>>> db;
...
int remove(int sid) {
    int count = 0;
    for(unordered_map<int, pair<int, pair<string,vector<double>>>>::iterator x = db.begin(); x != db.end(); ++x) {
        if(x->first == sid) {
            count++;
            db.erase(x->first);
            break;
...
int remove(string name) {
    int count = 0;
    for(unordered_map<int, pair<int,pair<string,vector<double>>>>::iterator it = db.begin(); it != db.end(); ++it) {
...
int adding(int sid, int cid, string name, double scoreC, double scoreM, double scoreE, double scoreP) {
    ...
    //db.insert(sid, make_pair(cid, make_pair(name, v)));
    db[sid] = make_pair(cid, make_pair(name, v));
    return 0;
}
...
string qury(int sid) {
    int rank = 1;
    unordered_map<int, pair<int, pair<string, vector<double>>>>::iterator std = db.find(sid);
...
int main() {
    ...
    while(chck) {
        ...
        int sid, cid, scoreC, scoreM, scoreE, scoreP;
        string name;
        ...
        while(chc == '1') { // adding student
            ...
            cin >> sid >> cid >> name >> scoreC >> scoreM >> scoreE >> scoreP;
            if(sid == 0) break;
